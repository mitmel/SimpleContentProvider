...
public class M2MDBHelper extends DBHelper {
	...
	public M2MDBHelper(GenericDBHelper from, GenericDBHelper to, IdenticalChildFinder identicalChildFinder){
		mFromTable = from.getTable();
		mToTable = to.getTable();
		mJoinTable = genJoinTableName(mFromTable, mToTable);
	...
	public void createTables(SQLiteDatabase db){
		db.execSQL("CREATE TABLE "+mJoinTable + " ("
				+ M2MColumns._ID 			+ " INTEGER PRIMARY KEY,"
				+ M2MColumns.TO_ID   		+ " INTEGER"
				+ (AndroidVersions.SQLITE_SUPPORTS_FOREIGN_KEYS ?
						" REFERENCES "+ mToTable + "("+BaseColumns._ID+")"
						+ " ON DELETE CASCADE"
						: "")
				+ ","
				+ M2MColumns.FROM_ID 		+ " INTEGER"
				+ (AndroidVersions.SQLITE_SUPPORTS_FOREIGN_KEYS ?
						" REFERENCES "+ mFromTable + "("+BaseColumns._ID+")"
						+ " ON DELETE CASCADE"
						: "")
				+ ");");
	}
	...
	public int removeRelation(SQLiteDatabase db, long from, long to){
		return db.delete(mJoinTable,
				M2MColumns.TO_ID + "=? AND " + M2MColumns.FROM_ID + "=?",
	 ...
	 */
	public Uri insertItemWithRelation(SQLiteDatabase db, ContentProvider provider, Uri parentChildDir, ContentValues values) {
		...
		try {
			...
			if (mIdenticalChildFinder != null){
				newItem = mIdenticalChildFinder.getIdenticalChild(this, parentChildDir, db, mToTable, values);
			}else{
				newItem = null;
			}
	...
	public int deleteDir(SQLiteDatabase db, ContentProvider provider, Uri uri,String where, String[] whereArgs){
		...
		try {
			...
			// as this is m2m, we cannot delete the destination content so this is a bit different than the insert (which is really just a shortcut)
	...
	public Cursor queryTo(long fromId, SQLiteDatabase db, String[] toProjection, String selection, String[] selectionArgs, String sortOrder){
		...
		return db.query(mToTable
				...
				+ " ON " + mJoinTable+"."+M2MColumns.TO_ID + "=" + mToTable + "." + BaseColumns._ID,
				ProviderUtils.addPrefixToProjection(mToTable, toProjection),
				ProviderUtils.addExtraWhere(selection, mJoinTable + "." + M2MColumns.FROM_ID + "=?"),
